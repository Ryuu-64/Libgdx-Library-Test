...
vec3 gaussianBlur(sampler2D texture) {
    ...
    for (int i=-kSize; i <= kSize; ++i)
    {
        ...
        {
            finalColor += kernel[kSize+i]*kernel[kSize+j]*texture2D(u_texture, v_texCoords.xy + vec2(float(i), float(j)) / vec2(300, 300)).rgb;// TODO vec2(300, 300) texture2d resolution
        }
    }
